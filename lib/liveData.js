...
function parseIndex(raw) {
  ...
  let m = raw.match(/(\d{14})~(-?[\d.]+)~(-?[\d.]+)~/)
  if (!m) m = raw.match(/(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})~(-?[\d.]+)~(-?[\d.]+)~/)
  if (!m || parts.length < 4) return null
  return {
    close: parseFloat(parts[3]) || 0,
    chg: parseFloat(m[3]).toFixed(2) + '%'
  }
}

function parseStock(raw) {
  // A股: ~现价(parts[3])~...~14位时间戳~涨跌额~涨跌幅~
  // 美股: ~现价(parts[3])~...~日期 时间~涨跌额~涨跌幅~
  let m = raw.match(/(\d{14})~(-?[\d.]+)~(-?[\d.]+)~/)
  if (!m) m = raw.match(/(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})~(-?[\d.]+)~(-?[\d.]+)~/)
  if (!m) return null
  const parts = raw.split('~')
